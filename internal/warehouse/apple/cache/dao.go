...
func read(id string) (cache *Cache, err error) {
	bytes, err := os.ReadFile(config.C.Resource.CachePath(id))
...
func write(cache *Cache) (err error) {
	...
	err = os.WriteFile(config.C.Resource.CachePath(cache.ID), bytes, 0666)
...
func remove(id string) (err error) {
	...
	err = os.Remove(config.C.Resource.CachePath(id))
	return
}

func readDirectory() (err error) {
	bytes, err := os.ReadFile(config.C.Resource.CacheDirectoryPath())
...
func writeDirectory() (err error) {
	...
	err = os.WriteFile(config.C.Resource.CacheDirectoryPath(), bytes, 0666)
	if err != nil {
		return
	}
...
func resetDirectory() (err error) {
	...
	dir, err := os.ReadDir(config.C.Resource.CacheDir())
