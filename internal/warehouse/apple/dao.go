...
func read(id string) (apple *Apple, err error) {
	file, err := os.ReadFile(config.C.Resource.ApplePath(id))
	if err != nil {
		return
	}
...
func write(apple *Apple) (err error) {
	...
	err = os.WriteFile(config.C.Resource.ApplePath(apple.ID), bytes, 0666)
	if err != nil {
		return
	}
...
func remove(id string) (err error) {
	err = os.Remove(config.C.Resource.ApplePath(id))
	if err != nil {
		return
	}
...
func readDirectory() error {
	config.C.Resource.AppleDir()
	file, err := os.ReadFile(config.C.Resource.AppleDirectoryPath())
	if err != nil {
		return err
	}
...
func writeDirectory() (err error) {
	...
	err = os.WriteFile(config.C.Resource.AppleDirectoryPath(), bytes, 0666)
	if err != nil {
		return
	}
...
func resetDirectory() (err error) {
	...
	// 1. 读取文件夹下所有的文件名
	dir, err := os.ReadDir(config.C.Resource.AppleDir())
