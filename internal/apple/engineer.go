...
import (
	...
	"githup.com/apibe/yifu/internal/apple/roads"
	"net/http"
	"strings"
	"time"
)

func (a *Apple) AutoAssembly(catch map[string]string, timeOut time.Duration) (*roads.Response, bool, error) {
	req := &roads.Request{}
	req.Header = make(map[string]string) // 初始化map
	err := a.valueReplace(catch)
	if err != nil {
		return roads.NewRequestErr("请求与接口配置不符！"), false, err
	}
	...
	if bytes, err := a.Cache.CacheGet(); err == nil {
		return roads.CacheResponse(bytes), true, err
	}
	// 3. 拼装url
	a.assemblyUrl(req)
	// 4. 拼装 Content-Type
